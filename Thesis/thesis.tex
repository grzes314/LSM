...
\begin{algorithm}
 \begin{algorithmic}[1]
  \Function{LSM-Put}{$N$, $K$, $M$, $S_0$, $\sigma$, $r$, $T$, $E$ }
    \State $S \gets$ matrix $N \times (K+1)$ \Comment{{\color{comment} $S[i,k]$ is the asset price at $i$\textsuperscript{th} path at time $t_k$.}}
    ...
    \For{$i = 1$ {\bf to} $N$} \Comment{{\color{comment} Generating trajectories like in Algorithm \ref{alg:single-tr}}}
      ...
      \State $CF[i,1] \gets T$ \Comment{{\color{comment} Realized cash flows conditional on }}
...
\begin{prop}
 The computational complexity of the LSM algorithm equals $\mathcal{O}(KNM + KM^3)$, where $N$ is the number of simulated paths, $M$ is the degree of approximating polynomial, $K$ is the number of time steps.
\end{prop}
