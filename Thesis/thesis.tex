...
Valuation of derivatives whose payoff depends on many assets does not differ much from one dimensional case. We only have to remember to take into account correlation between the assets. 



\begin{algorithm}
 \begin{algorithmic}[1]
  %\Comment{ $S_0$ and $\sigma$ are now tables, for example $\sigma[3]$ is the volatility of third asset.}
  \Function{Trajectory}{$S_0$, $\sigma$, $r$, $T$, $K$}
  \State S $\gets$ tow dimensional array \Comment{ S[i,k] is the price of i-th asset at k-th time point. }
  \For{$i=1$ {\bf to} $d$}
