...
\begin{figure}[h]
\centering
 \includegraphics[scale=0.4]{images/boxMonteCarlo.pdf}
...
\begin{figure}
\centering
 \includegraphics[scale=0.4]{images/convergenceMC.pdf}
...
\begin{figure}[!ht]
\centering
 \includegraphics[scale=0.4]{images/normalCorrelated.pdf}
...
Let $H = g(S_T)$ and $E$ be the strike price. For instance, if $H$ is a call option $g(x) = (x - E)_+$, and if $H$ is a put $g(x) = (E - x)_+$, but in fact $H$ might be any claim, whose payoff depends only on $S_T$. Since $S_T$ is also a function of some $Z \sim \mathcal{N}(0,T)$, thus actually $H = g(S_T(Z)) =: f(Z)$, for $f = g \circ S_T$. By $Z_i, i = 1,2,...$, we will denote replications of $Z$.
