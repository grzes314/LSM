...
Let $H = g(S_T)$ and $E$ be strike price. For instance, if $H$ is a call option $g(x) = (x - E)_+$, and if $H$ is put $g(x) = (E - x)_+$, but in fact $H$ might be any claim whose payoff depends only on $S_T$. Since $S_T$ is also a function of some $Z \sim \mathcal{N}(0,T)$, thus $H = f(Z)$, for some $f$. By $Z_i, i = 1,2,...$, we will denote replications of $Z$.
Crude Monte Carlo estrimator has the form:
\begin{equation}
 \label{eq:vanilla_CMC}
 \CMCa[H, 2n] = \frac{1}{2n}\sum\limits_{i=1}^{2n} f(Z_i).
\end{equation}
We will also use antithetic variates, where antithetic variable to $f(Z_i)$ will be  $f(-Z_i)$. 
\begin{equation}
 \label{eq:vanilla_AV}
 \AVa[H, n] = \frac{1}{n}\sum\limits_{i=1}^{n} \frac{f(Z_i) + f(-Z_i)}{2}.
\end{equation}
To use control variates method recall that $S_0 = \Em[S_T]$.
\begin{equation}
 \label{eq:vanilla_CV}
 \CVa[H, n] = \frac{1}{n}\sum\limits_{i=1}^{n} \left( f(Z_i) + c (S_T(Z_i) - S_0) \right),
\end{equation}
where $c$ is a value calculated from equation (\ref{eq:CVc}).
\begin{example}
To get a grip on using above estimators in practice, we will present how exactly looks an algorithm for pricing call options with antithetic variates method. Argument of the algorithm is \texttt{n} -- number of simulations. \medskip \\
\texttt{
sum <- sum\_sq <- 0\\
for i from 1 to n do\\
\hspace*{10pt} Z <- genereate blabbla\\
\hspace*{10pt} Spos <- S\_0 * exp\{ (r - 0.5 * vol*vol ) * T + vol * Z \}\\
\hspace*{10pt} Sneg <- S\_0 * exp\{ (r - 0.5 * vol*vol ) * T - vol * Z \}\\
\hspace*{10pt} Hpos <- max(Spos - E, 0) * exp\{-r*T\}\\
\hspace*{10pt} Hneg <- max(Sneg - E, 0) * exp\{-r*T\}\\
\hspace*{10pt} H <- 0.5 * (Hpos + Hneg)\\
\hspace*{10pt} sum <- sum + H \\
\hspace*{10pt} sum\_sq <- sum\_sq + H*H \\
sd <- sqrt( (sum\_sq - sum*sum) / (n*(n-1)) ) \\
se <- sd / sqrt(n)\\
price <- sum / n \medskip \\
}
Values calculated at the end are standard deviation, standard error and price of the option.
\end{example}

Table \ref{tab:vanilla} presents results of pricing using all of the above estimators.
\begin{table}[h]
\centering
 \caption{Comparision of pricing methods. Parameters of the model where $S = 100,\ \sigma=0.2,\ r = 0.05,\ E = 100$}
 \label{tab:vanilla}
\begin{tabular} {||c | c | c | c | c |c | c ||}  
 \hline 
  & \multicolumn{2}{|c|}{ CMC } & \multicolumn{2}{|c|}{ AV } & \multicolumn{2}{|c|}{ CV } \\
...
100    & TODO & TODO & TODO & TODO & TODO & TODO \\ \hline 
1000   & TODO & TODO & TODO & TODO & TODO & TODO \\ \hline 
\end{tabular}  
\end{table}
