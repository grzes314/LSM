...
The most popular way is to use the \textbf{Box-Muller algorithm} which uses two independent variates with the uniform distribution on $(0,1)$, and ``produces'' two independent variates with the standard normal distribution.
\begin{algorithm}[!ht]
 \begin{algorithmic}[1]
  \Function{BoxMuller}{}
...
In \cite{london} we can find a remark that necessity of calculating sine and cosine may slow down the above algorithm. Another algorithm, \textbf{polar rejection}, is proposed.
\begin{algorithm}[!ht]
 \begin{algorithmic}[1]
