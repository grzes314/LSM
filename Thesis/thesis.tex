...
\begin{algorithm}[!ht]
 \begin{algorithmic}[1]
  \Function{GaussDistr}{\null}
    ...
    \Comment{{\color{comment} or $(N_1, N_2) \gets $ \Call{PolarRejection}{\null}}}
...
\end{prop}
The derivation of the above fact was intuitive, but we need a more general version.

\begin{prop}
 If $Z_1$ and $Z_2$ are independent random variables with the same distribution $\mathcal{N}(0,1)$, then for any covariance matrix
 $\Sigma = \left( \begin{array}{cc}
                      \sigma_{11}^2 & \sigma_{12}^2 \\
                      \sigma_{12}^2 & \sigma_{22}^2
                   \end{array} \right),$
 random vector $ N = \mu + LZ$, where
  $L ~= \left( \begin{array}{cc}
                      \sigma_{11} & 0 \\
                      \frac{\sigma_{12}^2}{\sigma_{11}} & \sqrt{\sigma_{22}^2 - \frac{\sigma_{12}^4}{\sigma_{11}^2}}
                   \end{array} \right)$,
 has distribution $\mathcal{N}(\mu,\Sigma)$. Moreover, $\Sigma = LL^T$.
\end{prop}
\begin{proof}
Let's focus on the case $\mu_1 = \mu_2 = 0$, since the mean may be added at the end. We have $Z \sim \mathcal{N}(\bf{0}, \mathbb{I})$. By making an assigment $N_1 = \sigma_{11} Z_1$, $N_1$ has the desired variance. As previously, we look for $N_2$ of the form $N_2 = a Z_1 + b Z_2$. We want the variance of $N_2$ to equal $\sigma_{22}^2$, so
\[ \sigma_{22}^2 = \Var(N_2) = \Var( a Z_1) +  \Var(b Z_2) = a^2 + b^2 \]
Moreover
\[ \sigma_{12}^2 = \Cov(N_1, N_2) = \sigma_{11} a\Cov(Z_1, Z_1) + \sigma_{11} b\Cov(Z_1, Z_2) = a\sigma_{11} \]
Hence
\[ a = \frac{\sigma_{12}^2}{\sigma_{11}},\ \ \ \ b = \sqrt{\sigma_{22}^2 - \frac{\sigma_{12}^4}{\sigma_{11}^2}}. \] 
Equation $\Sigma = LL^T$ may be proven by simple matrix multiplication.
\end{proof}

It turns out that the above Proposition can be generalized for higher dimensions.
%\begin{thm}
% For the positive-definite matrix $\Sigma$ there 
%\end{thm}


That explains following algorithm for generating values from multivariate normal distribution with given mean vector $\mu$ and positive-definite covariation matrix $\Sigma$. 
\begin{enumerate}
